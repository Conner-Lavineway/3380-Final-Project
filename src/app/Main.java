...
// start up, connect to SQL Server, then dispatch menu actions.
public final class Main {
  ...
  // Analyst queries are kept in a catalog so the menu stays simple here.
  ...
  private static void browseTables(DatabaseClient database, ConsolePrompter prompter) throws SQLException {
    ...
    while (true) {
      ...
      CachedTable rows = database.browseTable(table.id(), ProjectSchema.TABLE_ORDER_BY.get(table.id()), pageSize,
          offset);
      System.out.println();
  ...
  // The loader does the destructive rebuild work; Main only confirms and calls
  // it.
  ...
  private static DatabaseClient.Option chooseTable(DatabaseClient database, ConsolePrompter prompter)
      throws SQLException {
