...
public final class SprintVsRegularSummaryQuery extends QueryAction {
    ...
    public void run(DatabaseClient database, ConsolePrompter prompter) throws SQLException {
        QuerySupport.runAndPrint(
                ...
                SELECT TOP 20
                       rw.year,
                       COUNT(DISTINCT rw.round) AS total_weekends,
                       SUM(CASE WHEN sw.year IS NULL THEN 0 ELSE 1 END) AS sprint_weekends,
                       SUM(CASE WHEN sw.year IS NULL THEN 1 ELSE 0 END) AS regular_weekends,
                       COUNT(DISTINCT re.entry_id) AS race_entries,
